...
const PlaceDetail = () => {
  ...
  const thingsToDo = Array.isArray(place.things_to_do)
      ? place.things_to_do
      : typeof place.things_to_do === 'string'
          ? place.things_to_do.split(',').map((item) => item.trim()).filter(Boolean)
          : [];
          ...
          {thingsToDo.length > 0 ? (
              <Grid container spacing={1}>
                {thingsToDo.map((item, index) => (
                    <Grid item key={index}>
                      <Chip label={item} sx={{ mr: 1, mb: 1 }} /> {/* Chip мәтінін локализациялау үшін, API-да things_to_do_kz/en керек */}
                    </Grid>
                ))}
              </Grid>
          ) : (
